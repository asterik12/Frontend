...
export default class Dashboard extends Component {
  ...
  componentDidMount() {
    ...
    if (!currentUser) {
      ...
      taskService.getALLSharedTask().then(
        response => {
          this.setState({
            getALLSharedTask:response.data
          });
        }
      );
  ...
  componentDidUpdate() {
    ...
    if(this.state.getALLSharedTaskStatus) {
      taskService.getALLSharedTask().then(
        response => {
          this.setState({
            getALLSharedTask:response.data,
  ...
  render() {
    ...
    let a = new Set();
    let b = new Set();
    let LCount1 = new Array();
    let CCount1 = new Array();
    let FCount1 = new Array();
      ...
      <div className="container">
        <header className="jumbotron">
          ...
          <button type="button" class="btn btn-primary">
            Tasks <span class="badge badge-light">{this.state.task.length + this.state.getALLSharedTask.length}</span>
        ...
        <div class="card-group">
          ...
          <div class="row">
          {this.state.getALLSharedTask.map( (e, index) =>{
                return(
                ...
                <div class="col-sm-6 col-md-4" 
                  ...
                  {a.clear()}
                  {b.clear()}
                  {LCount1.splice(0, LCount1.length)}
                  {CCount1.splice(0, CCount1.length)}
                  {FCount1.splice(0, FCount1.length)}
                  ...
                  <div class="card-footer-actions">
                    ...
                    <Link to={`/read/${e.id}`} >
                     <span class= "show-action-content">
                       {this.state.likes.map((c) => {
                         return(
                          (c.taskId === e.id) ? (LCount1.push("") !== -1)?'':'' : ''
                         )
                       })}
                       {LCount1.length} likes
                      </span> 
                    </Link>  
                      &nbsp;&nbsp;
                    <Link to={`/read/${e.id}`} >
                      ...
                      {this.state.comments.map((c) => {
                         return(
                          (c.taskId === e.id) ? (CCount1.push("") !== -1)?'':'' : ''
                         )
                       })}
                       {CCount1.length} Comments
                    ...
                    <Link to={`/read/${e.id}`} >
                      ...
                      {this.state.follower.map((c) => {
                         return(
                          (c.taskId === e.id) ? (FCount1.push("") !== -1)?'':'' : ''
                         )
                       })}
                       {FCount1.length} followers
                      </span>
                    </Link>
                  ...
                  <div class="card-footer">
                    ...
                    <span>
                      ...
                      {this.state.likes.map((c) => {
                        return(
                            (c.taskId === e.id)&& (c.userId === currentUser.id) ? a.add("") : ''
                    ...
                    <span>
                      ...
                      {this.state.follower.map((foo) => {
                        return(
                            (foo.taskId === e.id)&& (foo.userId === currentUser.id) ? b.add("") : ''
